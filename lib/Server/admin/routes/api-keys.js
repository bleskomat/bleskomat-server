...
const bleskomatConf = require('bleskomat.conf');
...
module.exports = function(app) {
	...
	app.get('/admin/api-keys/:id/delete',
		function(req, res, next) {
			const { id } = req.params;
			config.lnurl.auth.apiKeys = config.lnurl.auth.apiKeys.filter(apiKey => {
				return apiKey.id !== id;
			});
			return env.save(config).then(() => {
				return res.redirect('/admin/overview');
	...
	app.get('/admin/api-keys/:id/download-config',
		function(req, res, next) {
			const { id } = req.params;
			const apiKey = config.lnurl.auth.apiKeys.find(obj => obj.id === id);
			if (!apiKey) {
			...
			const output = bleskomatConf.stringify(data);
