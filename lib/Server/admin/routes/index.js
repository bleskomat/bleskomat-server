...
module.exports = function(app) {
	...
	app.use(['/admin*'],
		function(req, res, next) {
			...
			if ([req.url, req.baseUrl].indexOf('/admin/setup') === -1 && !config.admin.password) {
		...
		function(req, res, next) {
			...
			if ([req.url, req.baseUrl].indexOf('/admin/login') === -1 && config.admin.password) {
		...
		function(req, res, next) {
			...
			res.render = function(filePath, context, options) {
				...
				if (context.nav && context.nav.items) {
					context.nav.items = context.nav.items.map(item => {
