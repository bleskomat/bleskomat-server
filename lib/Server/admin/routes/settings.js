...
const { checkBackend, formGroups } = require('lightning-backends');
...
module.exports = function(app) {
		...
		lightning: {
			form: new Form({
				...
				groups: [
					{
						...
						inputs: [{
							...
							type: 'select',
							options: (function() {
								return [{ key: '', label: '' }].concat(formGroups.map(formGroup => {
									const { name, label } = formGroup;
									return {
										key: name,
										label: label || name,
									};
							...
							required: true,
							validate: function(value) {
								if (!formGroups.find(formGroup => formGroup.name === value)) {
						...
						inputs = inputs.map(input => {
							return Object.assign({}, input, {
								...
								required: input.required !== true ? false : function(data) {
									return typeof data === 'undefined' || data.backend === name;
								},
							});
						});
						return { name, inputs };
				...
				validate: function(data) {
					...
					const { backend } = data;
					config.torSocksProxy = config.torSocksProxy;
					return checkBackend(backend, config).then(checksPassed => {
						if (!checksPassed) {
	...
	const formValuesToLightningConfig = function(values) {
		...
		switch (backend) {
			case 'lnd':
				config = _.omit(config, 'fingerprint', 'fingerprint256');
