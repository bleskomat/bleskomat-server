...
const LightningBackendsValidationError = createForm.ValidationError;
		...
		app.post(uri,
			...
			function(req, res, next) {
				return form.validate(req.body).then(newValues => {
					...
					if (error instanceof ValidationError || error instanceof LightningBackendsValidationError) {
