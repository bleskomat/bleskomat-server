...
const { createHash, getTagDataFromPaymentRequest, HttpError } = require('lnurl/lib');
	...
	app.get('/lnurlpos/:id', function(req, res, next) {
		return Promise.resolve().then(() => {
			...
			return lnurlServer.fetchUrl(hash).then(fetchedUrl => {
				...
				res.status(200).json(_.extend({}, { callback, tag },
					_.pick(params, 'minSendable', 'maxSendable', 'metadata'),
				));
			});
		}).catch(next);
	});
