...
module.exports = function(config) {
	...
	if (config.admin.web) {
		...
		if (!config.admin.bcrypt) {
			throw new Error('Missing required config: "admin.bcrypt"');
		}
		if (!_.isObject(config.admin.bcrypt)) {
			throw new Error('Invalid config ("admin.bcrypt"): Object expected');
		}
		if (!config.admin.bcrypt.saltRounds) {
			config.admin.bcrypt.saltRounds = 11;
		}
